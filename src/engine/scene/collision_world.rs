...
use nalgebra::{Translate, Cross, Translation, Rotation};
use ncollide_math::{Point, Vector, Isometry};
use ncollide::utils::data::uid_remap::{UidRemap, FastKey};
use ncollide::utils::data::vec_map::Values;
use ncollide_geometry::bounding_volume::{self, BoundingVolume, AABB};
use ncollide_geometry::shape::ShapeHandle;
use ncollide_geometry::query::{RayCast, Ray, RayIntersection, PointQuery};
use ncollide::narrow_phase::{NarrowPhase, DefaultNarrowPhase, DefaultContactDispatcher, DefaultProximityDispatcher,
                   ContactHandler, ContactPairs, Contacts, ContactSignal, ProximityHandler,
                   ProximitySignal, ProximityPairs};
use ncollide::broad_phase::{BroadPhase, DBVTBroadPhase, BroadPhasePairFilter, BroadPhasePairFilters};
use ncollide::world::{CollisionObject, GeometricQueryType, CollisionGroups, CollisionGroupsPairFilter};
...
pub struct CollisionWorld<P: Point, M, T> {
    ...
    pos_to_update:     Vec<(FastKey, M, Option<AABB<P>>)>,
...
impl<P, M, T> CollisionWorld<P, M, T>
    ...
    pub fn deferred_set_position(&mut self, uid: usize, pos: M) {
        if let Some(fk) = self.objects.get_fast_key(uid) {
            self.pos_to_update.push((fk, pos, None))
        }
    }

    pub fn deferred_set_position_bv(&mut self, uid: usize, pos: M, aabb: AABB<P>) {
        if let Some(fk) = self.objects.get_fast_key(uid) {
            self.pos_to_update.push((fk, pos, Some(aabb)))
        }
    }
    ...
    pub fn perform_position_update(&mut self) {
        ...
        mem::swap(&mut self.pos_to_update, &mut updates);

        for (fk, pos, aabb_opt) in updates.into_iter() {
